...
class DialoGPTGenerator:
    ...
    def generate_response(self, prompt, model_name="dialogpt_onnx", target_length=None, max_new_tokens=None):
        ...
        try:
            ...
            print("🔍 Спроба генерації через Triton сервер...")
            ...
            for step in range(max_new_tokens):
                ...
                if next_token_id == tokenizer.eos_token_id:
                    if len(generated_tokens) >= self.min_tokens_before_eos:
                        break
                    ...
                    if step < max_new_tokens - 1:
                        try:
                            ...
                            if len(first_pad_pos) > 0:
                                ...
                                input_ids = encode_with_padding(current_input, target_length)
                                logits = self.run_inference(client, model_name, input_ids)
                        except Exception as inner_e:
                            print(f"⚠️ Inner inference error: {inner_e}")
                            break
            ...
            print(f"✅ TRITON УСПІШНО: '{response}' ({len(generated_tokens)} токенів)")
            return response
